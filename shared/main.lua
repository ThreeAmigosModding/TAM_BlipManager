...
assert(lib.checkDependency("ox_lib", '3.27.0') ~= nil, "ox_lib is outdated! Please ensure you upgrade to the latest version! \n https://github.com/overextended/ox_lib/releases/latest")
...
local function printConsoleMsg(fail, reason)
    lib.print.info("-----------------------------------------------")
    lib.print.info("Loading TAM_BlipManager")
    ...
    elseif not fail then
        lib.locale()
        lib.print.info("Successfully loaded TAM_BlipManager!")
...
if GetCurrentResourceName() ~= "TAM_BlipManager" then
    printConsoleMsg(true, "Resource must be named TAM_Fuel to function properly! Aborting loadup.")
elseif not GetResourceMetadata("TAM_BlipManager", "author", 0) == "ThreeAmigosModding" then
...
elseif not LoadResourceFile('TAM_BlipManager', 'locales/en.json') then
...
if lib.context == "SERVER" and Config.versionCheck then lib.versionCheck('threeamigosmodding/TAM_BlipManager') end